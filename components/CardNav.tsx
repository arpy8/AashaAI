...
import { MoveUpRight, CircleAlert } from "lucide-react";
...
const CardNav: React.FC<CardNavProps> = ({
  ...
  return (
    <div
      className={`absolute left-1/2 -translate-x-1/2 w-[90%] max-w-[800px] z-[99] top-[1em] md:top-[1em] ${className}`}
    >
      <nav
        ref={navRef}
        className={`card-nav ${
      ...
      >
        <div className="absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] z-[2]">
          ...
          <Button variant="destructive">
            <CircleAlert />{" "}
            <Link href="https://www.youtube.com/watch?v=6p_yaNFSYao&pp=ygURZ3VpZGVkIG1lZGl0YXRpb24%3D">Panic Button</Link>
          </Button>
        </div>
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className="border border-gray-800 rounded-lg backdrop-blur-lg nav-card select-none relative flex flex-col gap-2 p-[12px_16px] rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%]"
              ref={setCardRef(idx)}
